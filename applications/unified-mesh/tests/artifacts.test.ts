...
describe('Artifacts', () => {
  it('should execute queries', async () => {
    ...
    expect(
      executionResult?.data?.me?.company?.employers[0]?.firstName
    ).toBeDefined();
    expect(
      executionResult?.data?.me?.company?.employers[0]?.jobTitle
    ).toBeDefined();
    expect(
      executionResult?.data?.me?.company?.employers[0]?.lastName
    ).toBeDefined();
  }, 12000);

  it('should load and run SDK correctly', async () => {
    const { getMeshSDK } = await import('../.compiled/.mesh/index');
